...
```{r}
#Customer info
hist1 <- ggplot(data = sub1_2, mapping = aes(x = Age, colour = group)) + geom_freqpoly(binwidth = 3) + ggsave("figures/hist/histAge.png")

hist2 <- ggplot(data = sub1_2, mapping = aes(x = L_O_S, colour = group)) + geom_freqpoly(binwidth = 3) + ggsave("figures/hist/histLOS.png")

# Tarifs schemes
hist20 <- ggplot(data = sub20_24, mapping = aes(x = call_cost_per_min, colour = group)) + geom_freqpoly(binwidth = 3) + ggsave("figures/hist/hist_call_cost_per_min.png")

hist21 <- ggplot(data = sub20_24, mapping = aes(x = actual.call.cost, colour = group)) + geom_freqpoly(binwidth = 3) + ggsave("figures/hist/hist_actual.call.cost.png")

hist22 <- ggplot(data = sub20_24, mapping = aes(x = Total_call_cost, colour = group)) + geom_freqpoly(binwidth = 3) + ggsave("figures/hist/hist_Total_call_cost.png")

hist23 <- ggplot(data = sub20_24, mapping = aes(x = Total_Cost, colour = group)) + geom_freqpoly(binwidth = 3) + ggsave("figures/hist/hist_Total_Cost.png")

hist24 <- ggplot(data = sub20_24, mapping = aes(x = average_cost_min, colour = group)) + geom_freqpoly(binwidth = 3) + ggsave("figures/hist/hist_average_cost_min.png")
```
Don't know if it gives us a lot of information..
