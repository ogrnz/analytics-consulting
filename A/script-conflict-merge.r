...
#OG
setwd("E:/Nextcloud/Ecole/UNI/master/Analytics consulting/case studies/A")
...
clothing %>% ggplot(aes(x=ClothingType,
  ...
  theme(legend.position = "none") +
  theme_minimal() +
  labs(title="Relation between clothing type and number of interactions",
       x="Clothing type",
       y="Number of interactions",
       caption="Figure 4") +
  guides(fill = FALSE)+
...
fem <- clothing[which(Gender=="Female"),]
mal <- clothing[which(Gender=="Male"),]

unis <- clothing[which(ClothingType=="Unisex"),]
std <- clothing[which(ClothingType=="Std"),]
other <- clothing[which(ClothingType=="Other"),]
mean(unis$Count)
mean(std$Count)
mean(other$Count)

# Sum of positive interactions for females 
sum(fem[which(InteractionType=="PosInt"),]$Count, na.rm=TRUE)

# Sum of positive interactions for females 
sum(mal[which(InteractionType=="PosInt"),]$Count, na.rm=TRUE)
